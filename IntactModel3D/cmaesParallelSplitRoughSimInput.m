...
for i = 1:numTerrains:(numTerrains*popSize)
    ...
    in(i) = in(i).setVariable('GainFHABst',               Gains(39),'Workspace',model);
    in(i) = in(i).setVariable('GainPhiHATHABst',          Gains(40),'Workspace',model);
    in(i) = in(i).setVariable('GainDphiHATHABst',         Gains(41),'Workspace',model);
    in(i) = in(i).setVariable('GainPhiHATHADst',          Gains(42),'Workspace',model);
    in(i) = in(i).setVariable('GainDphiHATHADst',         Gains(43),'Workspace',model);
    in(i) = in(i).setVariable('GainSHABcHABst',           Gains(44),'Workspace',model);
    in(i) = in(i).setVariable('GainSHADcHADst',           Gains(45),'Workspace',model);
    in(i) = in(i).setVariable('GainLHABsw',               Gains(46),'Workspace',model);
    in(i) = in(i).setVariable('GainLHADsw',               Gains(47),'Workspace',model);
    in(i) = in(i).setVariable('legLengthClr',             Gains(48),'Workspace',model);
    in(i) = in(i).setVariable('simbiconLegAngle0',        Gains(49),'Workspace',model);
    in(i) = in(i).setVariable('simbiconGainD',            Gains(50),'Workspace',model);
    in(i) = in(i).setVariable('simbiconGainV',            Gains(51),'Workspace',model);
    in(i) = in(i).setVariable('phiHATref',                Gains(52),'Workspace',model);
    in(i) = in(i).setVariable('deltaLegAngleThr',         Gains(53),'Workspace',model);
    in(i) = in(i).setVariable('simbiconLegAngle0_C',      Gains(54),'Workspace',model);
    in(i) = in(i).setVariable('simbiconGainD_C',          Gains(55),'Workspace',model);
    in(i) = in(i).setVariable('simbiconGainV_C',          Gains(56),'Workspace',model);
    in(i) = in(i).setVariable('transSupst',               Gains(57),'Workspace',model);
    in(i) = in(i).setVariable('transsw',                  Gains(58),'Workspace',model);
    in(i) = in(i).setVariable('transSupst_C',             Gains(59),'Workspace',model);
    in(i) = in(i).setVariable('transsw_C',                Gains(60),'Workspace',model);
    in(i) = in(i).setVariable('PreStimHFLst',             Gains(61),'Workspace',model);
    in(i) = in(i).setVariable('PreStimGLUst',             Gains(62),'Workspace',model);
    in(i) = in(i).setVariable('PreStimHAMst',             Gains(63),'Workspace',model);
    in(i) = in(i).setVariable('PreStimRFst',              Gains(64),'Workspace',model);
    in(i) = in(i).setVariable('PreStimVASst',             Gains(65),'Workspace',model);
    in(i) = in(i).setVariable('PreStimBFSHst',            Gains(66),'Workspace',model);
    in(i) = in(i).setVariable('PreStimGASst',             Gains(67),'Workspace',model);
    in(i) = in(i).setVariable('PreStimSOLst',             Gains(68),'Workspace',model);
    in(i) = in(i).setVariable('PreStimTAst',              Gains(69),'Workspace',model);
    in(i) = in(i).setVariable('PreStimHABst',             Gains(70),'Workspace',model);
    in(i) = in(i).setVariable('PreStimHADst',             Gains(71),'Workspace',model);
    in(i) = in(i).setVariable('PreStimHFLsw',             Gains(72),'Workspace',model);
    in(i) = in(i).setVariable('PreStimGLUsw',             Gains(73),'Workspace',model);
    in(i) = in(i).setVariable('PreStimHAMsw',             Gains(74),'Workspace',model);
    in(i) = in(i).setVariable('PreStimRFsw',              Gains(75),'Workspace',model);
    in(i) = in(i).setVariable('PreStimVASsw',             Gains(76),'Workspace',model);
    in(i) = in(i).setVariable('PreStimBFSHsw',            Gains(77),'Workspace',model);
    in(i) = in(i).setVariable('PreStimGASsw',             Gains(78),'Workspace',model);
    in(i) = in(i).setVariable('PreStimSOLsw',             Gains(79),'Workspace',model);
    in(i) = in(i).setVariable('PreStimTAsw',              Gains(80),'Workspace',model);
    in(i) = in(i).setVariable('PreStimHABsw',             Gains(81),'Workspace',model);
    in(i) = in(i).setVariable('PreStimHADsw',             Gains(82),'Workspace',model);
